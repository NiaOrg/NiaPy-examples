...
def fillprototype(f, restype, argtypes): f.restype, f.argtypes = restype, argtypes
...
class MinMB(Benchmark):
   ...
   def function(self): return lambda d, x: self.run_fun(asarray(x).ctypes.data_as(ctypes.POINTER(ctypes.c_double)), ctypes.c_int(d), ctypes.c_int(self.fnum))
