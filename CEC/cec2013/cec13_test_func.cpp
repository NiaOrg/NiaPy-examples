...
double runtest(double *x, int s, int fnum) {
	double f;
	return *cec13_test_func(x, &f, s, 1, fnum);
}

double* cec13_test_func(double *x, double *f, int nx, int mx,int func_num) {
	int cf_num=10,i;
	if (ini_flag==1) {
	...
	if (ini_flag==0) {
		...
		sprintf(FileName, "cec2013/input_data/M_D%d.txt", nx);
		...
		fpt=fopen("cec2013/input_data/shift_data.txt","r");
		if (fpt==NULL) printf("\n Error: Cannot open input file for reading \n");
		OShift=(double *)malloc(nx*cf_num*sizeof(double));
		if (OShift==NULL) printf("\nError: there is insufficient memory available!\n");
		for(i=0;i<cf_num*nx;i++) fscanf(fpt,"%lf",&OShift[i]);
		fclose(fpt);
